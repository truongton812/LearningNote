...
#### 3.3.3. ovn driver
- ovn driver vẫn dùng OVS bridge, nhưng đơn giản hơn so với openvswitch driver vì loại bỏ được br-tun, do ovn-controller tự lập trình Geneve tunnel trực tiếp vào br-int, không cần một bridge riêng để xử lý tunnel (VXLAN/GRE) nữa
...
Kiến trúc Neutron chia thành 4 tầng
...
- Kiến trúc hiện đại chia plugin thành hai tầng: core plugin (ML2 — Modular Layer 2) xử lý network/subnet/port, và service plugin xử lý router, firewall, VPN, LB… Bên trong ML2 tách thành 3 loại driver: TypeDriver (flat, VLAN, VXLAN, GRE) và mechanism driver (openvswitch, linuxbridge, OVN, SR-IOV) để tách biệt loại mạng và cách triển khai.
  —  (VXLAN, VLAN, GRE...), MechanismDriver (OVS, OVN, LinuxBridge...), ExtensionDriver (port security, QoS...).
ML2 Plugin (Core Plugin hiện đại):
Service Plugins: L3 Router, LBaaS, FWaaS, VPNaaS — mỗi plugin là một API extension độc lập.

Tầng 3 — Agents: Chạy trên Network Node và Compute Node, lắng nghe message queue và thực thi cấu hình thực tế trên OS:
...
Tầng 4 — Data Plane: Packet thực sự chạy qua đây. Với OVS:
...
### 4.1. Core Plugin (ML2 - Modular Layer 2)

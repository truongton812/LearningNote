...
- Là ứng dụng để tương tác với GCP bằng commnad line
- Các lệnh cơ bản

`gcloud init` : khởi tạo hoặc tái khởi tạo gcloud (khai báo default project, default zone,...)

`gcloud config list` : list ra các thuộc tính của config hiện tại (region, zone, account, project,...)

`gcloud config set project <project_name>`: set project

`gcloud compute instances list` : list ra các VM instance

`gcloud compute zones list` : list ra các zone

`gcloud compute regions list` : list ra các region

`gcloud compute machine-types list` : list ra machine type

`gcloud compute machine-types list --filter="zone:us-central1-b"` : list ra machine type dựa theo 1 zone

`gcloud compute machine-types list --filter="zone:( us-central1-b europe-west1-d )"` : list ra machine type dựa theo nhiều zone


## CloudShell

- Google Cloud Shell là một môi trường dòng lệnh trực tuyến miễn phí mà Google cung cấp cho người dùng quản lý và phát triển các tài nguyên trên Google Cloud Platform (GCP) trực tiếp từ trình duyệt web. Cloud Shell của GCP sử dụng thông tin xác thực (credential) của chính tài khoản Google mà bạn đang đăng nhập vào Cloud Console
- Khi dùng Cloud Shell thực chất Google provision một máy chủ Linux đã cài sẵn các công cụ quản trị và phát triển như gcloud CLI, kubectl, Docker, Terraform, Git, cùng với 5GB dung lượng lưu trữ trong thư mục $HOME. Các file trong $HOME tồn tại kể cả khi đã đóng CloudShell session, tuy nhiên thời gian lưu trữ chỉ tối đa 120 ngày
- CloudShell có thể SSH vào VM instance bằng ip private

...
- Zone khi tạo K8S cluster có các options:
  - Zonal Cluster:
    - Single zone: control plane nằm trong cùng 1 zone với nodes
    - Multi-zonal: control plane trong 1 zone, các nodes có thể ở nhiều zone
  - Regional cluster: control plane có nhiều replicas nằm ở nhiều zone trong 1 region, các nodes cũng ở nhiều zone
  - Private cluster: cluster nằm trong VPC, nodes chỉ có internal IP
  - Alpha cluster: dùng để test tính năng mới của K8s
...
- Khi tạo service dạng load balancer, sẽ có 1 load balancer tương ứng sinh ra trên GCP. Best approach khi muốn expose các services ra internet là expose các services dưới dạng NodePort, sau đó tạo 1 ingress controller expose ra bằng type LoadBalancer -> ingress controller sẽ xử lý request theo path

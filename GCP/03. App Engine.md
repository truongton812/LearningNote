...
<img width="791" height="401" alt="image" src="https://github.com/user-attachments/assets/a79d6fb4-f8a5-4d98-a773-0d053d6254e6" />
...
|--------------------------|-----------------------------|-----------------------------|
| Chi phí | Tính theo instance mỗi giờ | Tính theo số vCPU, Memory và Persistent disk của instance |
| Scaling | Manual, Basic, Automatic | Manual, Automatic |
| Scaling về 0 | Có | Không, ít nhất phải có 1 instance |
| Startup instance         | Vài giây                    | Vài phút                    |
| Thời gian tối đa request  | Ngắn (từ 1 - 10 phút)    | Lên đến 60 phút             |
| Local disk | Dùng /tmp | Có, sử dụng disk của instance |
...
Giải thích thêm về các chế độ scaling:
- Automatic: tự động scale số lượng instances dựa trên CPU, Throughput hoặc số lượng request đồng thời. Phù hợp với workload running continuosly. Có thể config max instance và min instance
- Basic: tự động scale nhưng dùng cho các adhoc workloads không thể dự đoán trước. Có thể scale về 0 nếu không có request -> không được hỗ trợ khi dùng Flexible Environment. Có thể config max instance và idle timeout - là khoảng thời gian chờ trước khi scale về 0 instance nếu không có request đến. Chế độ này không có config min instance
- Manual: scale thủ công
  
---

Template app.yaml
```
runtime: python39              # Chọn runtime (ví dụ python39, nodejs16, java11, go116,...)

service: default               # Tên service, mặc định là 'default'

instance_class: F2             # Loại instance (chỉ dành cho Standard Environment)

entrypoint: gunicorn -b :$PORT main:app    # Lệnh khởi động app (thường dùng với flexible environment)

env: standard                 # Môi trường: 'standard' hoặc 'flex' (flexible environment)

automatic_scaling:             # Cấu hình tự động scale (Standard Env)
  min_idle_instances: 1        # Số instance tối thiểu luôn chạy
  max_idle_instances: 5        # Số instance tối đa khi idle
  max_concurrent_requests: 50 # Số request tối đa mỗi instance có thể xử lý

manual_scaling:                # Cấu hình manual scale (Flexible Env)
  instances: 2                # Số instance cố định

resources:                    # Cấu hình tài nguyên cho flexible environment
  cpu: 2
  memory_gb: 4
  disk_size_gb: 10

handlers:                     # Định nghĩa cách xử lý các URL path
- url: /static                 # Tài nguyên tĩnh (static files)
  static_dir: static           # Thư mục chứa static files

- url: /.*
  script: auto                # Tất cả các request còn lại là gửi tới app

env_variables:                # Biến môi trường cho app
  ENV_VAR_NAME: 'value'

health_check:                 # Kiểm tra sức khỏe instance
...
readiness_check:              # Kiểm tra sẵn sàng nhận traffic
...
liveness_check:               # Kiểm tra tính sống sót
  path: "/liveness_check"
  timeout_sec: 4

automatic_networks:           # Cấu hình mạng (thường dùng flexible env)
- name: default

```

Trong file app.yaml của Google App Engine, phần handlers dùng để định nghĩa cách mà các HTTP request đến URL cụ thể sẽ được xử lý. 

Mỗi phần tử handler thường bao gồm:

- url: Một biểu thức chính quy (regular expression) POSIX để xác định các URL phù hợp.

- Cách xử lý URL đó, có thể là:

    - script: Chỉ định file hoặc module mã nguồn để xử lý request (ví dụ Python module, file Python).

    - static_dir: Chỉ định thư mục chứa các file tĩnh (ảnh, css, js) để phục vụ trực tiếp khi request tương ứng.

    - static_files và upload: Chỉ định một file tĩnh cụ thể và file mẫu để upload.
...
handlers:
- url: /stylesheets
  static_dir: stylesheets  # Phục vụ file tĩnh trong thư mục stylesheets cho URL bắt đầu /stylesheets

- url: /images/(.*)
  static_files: images/\1  # Phục vụ file tĩnh với tên file động theo biểu thức regex
  upload: images/.*

- url: /admin/.*
  script: admin.app  # Các URL /admin/... được xử lý bởi module 'admin.app'
  login: admin      # Chỉ cho phép admin truy cập

- url: /.*
  script: main.app  # Các request còn lại được gửi cho module 'main.app'
```

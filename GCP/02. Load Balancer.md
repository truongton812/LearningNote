...
Serverless NEG (Network Endpoint Group)﻿ là một loại backend đặc biệt dùng cho Cloud Load Balancer﻿, cho phép bạn trỏ traffic từ Load Balancer trực tiếp tới các dịch vụ serverless như Cloud Run﻿, App Engine﻿, Cloud Run Functions﻿, hoặc API Gateway﻿ mà không cần máy ảo hoặc IP/port cụ thể.​

Đặc điểm chính:

- Không có endpoint dạng IP/port: Khác với NEG thông thường (trỏ tới VM, container trên GKE với IP cụ thể), Serverless NEG﻿ chỉ chứa thông tin về dịch vụ serverless (FQDN - tên miền đầy đủ của service Cloud Run﻿ chẳng hạn).​

- Một NEG = một hoặc nhiều resource: Mỗi Serverless NEG﻿ trỏ tới một service Cloud Run﻿ cụ thể, hoặc dùng URL mask﻿ để map nhiều service theo pattern (ví dụ: example.com/<service>).​

- Hỗ trợ nhiều loại Load Balancer: Global external Application Load Balancer﻿, Regional external/internal Application Load Balancer﻿, Cross-region internal Application Load Balancer﻿ đều dùng được Serverless NEG﻿.​

Tại sao cần Serverless NEG? Khi bạn muốn đặt Cloud Run﻿ sau Load Balancer﻿ để:

- Dùng custom domain và SSL certificate chung cho nhiều service.

- Kết hợp nhiều backend khác nhau (Cloud Run, GKE, VM...) trong cùng một LB, phân chia traffic theo URL path hoặc host.

- Tận dụng Cloud Armor﻿ (WAF), Cloud CDN﻿, hoặc Identity-Aware Proxy﻿ ở tầng LB.

- Tạo kiến trúc multi-region với failover tự động.

Thì bạn tạo Serverless NEG﻿ trỏ tới Cloud Run﻿ service, rồi gắn NEG đó làm backend của Load Balancer.​

Ví dụ đơn giản

Deploy Cloud Run﻿ service: my-service ở region us-central1.

Tạo Serverless NEG﻿ trỏ tới my-service.

Gắn NEG vào backend service﻿ của HTTPS Load Balancer﻿.

LB nhận request từ Internet → route tới Serverless NEG﻿ → NEG proxy tới Cloud Run﻿.

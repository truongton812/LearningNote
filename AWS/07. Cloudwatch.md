...
- Mặc định EC2 gửi metric đến Cloudwatch 5 phút 1 lần và chỉ gửi được các metrics như CPUUtilization, DiskReadOps, NetworkIn, StatusCheckFailed.
  - Nếu enable detailed EC2 monitoring có thể gửi metrics 1 phút 1 lần (mất phí). Detailed monitoring chỉ thay đổi tần suất thu thập metric mặc định của EC2, không mở rộng ra các metric hệ thống và cũng không thu thập nhiều metric hơn basic monitoring
  - Nếu cài UCA (Unified CloudWatch Agent) thì có thể collect thêm system-level metric (memory, disk usage, ...) và log. UCA còn có thể cài trên on-prem. Lưu ý khi cài đặt CloudWatch Agent cần phải cấu hình cụ thể những metric nào muốn thu thập và gửi đi. Nếu không cấu hình metric, agent có thể chỉ gửi metric mặc định rất cơ bản hoặc không gửi gì. File cấu hình của Cloudwatch Agent có thể tạo thủ công hoặc lấy từ SSM Parameter Store
- Example cấu hình cho Cloudwatch Agent để thu thập metric memory và disk
```
{
	"metrics": {
		"append_dimensions": {
...
- CloudWatch Alarm action có các target:
...
- Ví dụ tạo cloud alarm bằng terraform
```
# Tạo CloudWatch alarm cho Route53 health check
resource "aws_cloudwatch_metric_alarm" "api_health_check" {
  alarm_name          = "${var.project_name}-${var.environment}-api-health-check"
  comparison_operator = "LessThanThreshold" #support GreaterThanOrEqualToThreshold, GreaterThanThreshold, LessThanThreshold, LessThanOrEqualToThreshold
  ...
  statistic           = "Minimum"
  threshold           = 1
  alarm_description   = "API endpoint health check failed"
  alarm_actions       = [aws_sns_topic.alerts.arn] #optional, action khi có alarm
  ok_actions          = [aws_sns_topic.alerts.arn] #optional, action khi vào ok state

  dimensions = {
    HealthCheckId = aws_route53_health_check.api.id #dimensions để xác định metric
  }
...
- Để dùng CloudWatch log cần tạo log group (thường là 1 AWS service hoặc 1 application có 1 log group), trong log group tạo các log stream: mỗi 1 stream tương ứng với log của instances/files/containers.
  - Lệnh để kiểm tra dung lượng của các log group
	```
	aws logs describe-log-groups \
	  ...
	  | jq -s '
	    ...
	    | reverse[] 
	    | .logGroupName + " " 
	      + ((.storedBytes / 1073741824) | tostring) + " GiB"
	  '
	```
...
- Phân biệt log group và log stream
  - Log Group là thư mục/folder chứa nhiều log stream, có thể cấu hình chung Retention, IAM policy, metric filter. 1 log group chứa nhiều log stream
  - Log stream là chuỗi log events từ 1 nguồn cụ thể, là thư mục con trong log group. VD tất cả container trong ECS service gửi log vào cùng 1 Log Group, nhưng mỗi container/task tạo 1 Log Stream riêng, không bị lẫn log giữa các container khác nhau
...
- Trả tiền theo lượng data query
- Sử dụng SQL-like language
- Ví dụ
``` 
fields @timestamp, @message #Chọn hai trường timestamp và message (nội dung log) để hiển thị trong kết quả
| filter @message like /ERROR/ # chỉ lấy những log có chữ “ERROR” trong nội dung
| sort @timestamp desc # Sắp xếp kết quả theo trường @timestamp giảm dần, 
| limit 20 #Giới hạn chỉ hiển thị 20 dòng log đầu tiên
```
- Các command hay dùng: fields, filter, sort, limit, stats (dùng để tính thống kê như sum, average), parse (dùng để extract field từ unstructured log messages)
...
- Có thể aggregate logs từ multi-regions và multi-accounts. (có cách đơn giản hơn là vào Setting của Cloudwatch log và enable tính năng cross-account observeability)

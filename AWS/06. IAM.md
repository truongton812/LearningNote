...
---
### Tạo IAM role bằng terraform

IAM Role cần 2 thành phần: trust policy và permissions. 

Tạo role bằng resource aws_iam_role, trong role sử dụng trường assume_role_policy để định nghĩa trust policy(có thể viết trực tiếp bằng jsonencode() hoặc gán thông qua aws_iam_policy_document resource), sau đó attach policy bằng aws_iam_role_policy_attachment (managed policy) hoặc aws_iam_role_policy (inline).

- Ví dụ: tạo role cho EC2 với trust policy viết trực tiếp bằng jsonencode
```
...
- Ví dụ: tạo role cho EC2 với trust policy được gán thông qua aws_iam_policy_document resource
```
...
- aws_iam_policy_document là data source dùng để “lắp ghép” một IAM policy/trust policy bằng HCL và xuất ra JSON chuẩn, để đưa vào các resource cần policy JSON như aws_iam_policy.policy, aws_iam_role.assume_role_policy, aws_s3_bucket_policy.policy,…

#### Cách để gán permission cho IAM role

##### Gán thông qua incline policy
```
resource "aws_iam_role_policy" "inline" { #aws_iam_role_policy là inline policy gắn trực tiếp vào role.
...
##### Gán managed policy bằng aws_iam_role_policy_attachment

Lưu ý managed policy không nhất thiết phải là policy cho AWS tạo, người dùng cũng có thể tạo
```
resource "aws_iam_policy" "custom" {
  name        = "my-custom-policy"
  description = "Custom policy for app"

  policy = data.aws_iam_policy_document.custom.json #sử dụng aws_iam_policy_document để tạo policy
}
```
...
resource "aws_iam_role_policy_attachment" "s3_readonly" {
  ...
  policy_arn = aws_iam_policy.custom.arn # hoặ có thể dùng luôn AWS managed policy mà không cần tạo aws_iam_policy riêng, chỉ cần biết ARN (ví dụ arn:aws:iam::aws:policy/AmazonS3ReadOnlyAccess).
}
```

Ưu điểm của cách này:

- Policy là resource riêng (aws_iam_policy), có thể attach cho nhiều role.

- Role và policy tách biệt, dễ quản lý, dễ audit, dễ thay đổi.

##### Gán managed policy trực tiếp trên resource role
```
...
Hướng dẫn viết AWS IAM policy bằng 2 cách là HCL (dùng trong aws_iam_policy_document Terraform) và JSON (định dạng gốc của IAM policy).

1. Cấu trúc cơ bản của IAM policy
IAM policy luôn có dạng:
...
2. Viết IAM policy bằng HCL (Terraform): Terraform dùng aws_iam_policy_document để viết policy theo HCL, sau đó tự sinh ra JSON.
...
Các field HCL quan trọng:

- version: bắt buộc, thường là "2012-10-17".
- statement: mảng các block statement (mỗi statement là 1 rule).
- effect: "Allow" hoặc "Deny".
- actions: mảng các action, vd ["ec2:RunInstances", "s3:*"].
- resources: mảng ARN của tài nguyên.
- condition: (tuỳ chọn) thêm điều kiện, ví dụ giới hạn IP, thời gian, v.v.


Cách dùng trong Terraform nếu bạn muốn viết luôn JSON:

```
resource "aws_iam_policy" "example" {
  name = "ExampleS3ReadPolicy"
...
```
Ở đây jsonencode sẽ tự convert object HCL → JSON đúng chuẩn.

---

IAM policy chỉ chấp nhận đúng 1 element "Statement": [] - không có Statement1, Statement2 hay nhiều "Statement" lặp lại:

```
{
  "Version": "2012-10-17",
  "Statement": [     // CHỈ 1 "Statement" duy nhất, là array chứa tất cả blocks
    {}, {}, {}       // Tất cả statements gộp vào đây
  ]
}
```

Lỗi phổ biến: Multiple "Statement": [] hoặc tên khác → AWS báo "Syntax errors in policy" hoặc "More than one JSON statement element".

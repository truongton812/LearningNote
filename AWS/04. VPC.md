...
Phân biệt giữa NAT GW và NAT instance: NAT Gateway là dịch vụ managed, HA và băng thông cao; NAT Instance là EC2 tự quản lý (người dùng phải tự patch OS, update, monitoring, backup, tự triển khai HA nếu cần) băng thông phụ thuộc loại instance (t3.micro khác m5n.2xlarge), linh hoạt hơn và có thể rẻ hơn khi lưu lượng thấp.

Khác nhau về chi phí
NAT Gateway: tính phí theo giờ cho mỗi NAT Gateway đang “available”. Ví dụ nhiều region ~0.045 USD/giờ ≈ 32.85 USD/tháng/gateway (730h). Chưa kể chi phí dữ liệu đi qua NAT GW

NAT Instance: trả phí theo loại EC2: giá theo loại instance × giờ sử dụng. Không có “data processing fee” riêng cho NAT, chỉ chịu phí data transfer bình thường (Internet egress, cross-AZ...). Với lưu lượng thấp (ít GB), NAT Instance rẻ hơn rất nhiều so với NAT Gateway (3–8 USD vs 30–50+ USD/tháng).

Script install nat instance
```
#!/bin/bash
...
# Config iptables for NAT masquerading
INTERFACE=$(netstat -i | awk 'NR>2 {print $1}' | grep -E '^eth' | head -1)
...
- Private endpint:
	- Nếu dùng VPC Endpoint (private endpoint), traffic sẽ được chuyển bằng mạng private của AWS mà không đi qua internet hoặc NAT Gateway, giúp tiết kiệm chi phí hơn
 	- Khi dùng VPC Endpoint, với mỗi dịch vụ ta cần tạo 1 VPC Endpoint riêng (VD VPC Endpoint cho S3, VPC Endpoint cho RDS,...)
  	- Có thể dùng security group để control traffic ra vào Interface Endpoint. Ví dụ khi muốn Lambda trong private subnet kết nối đến Amazon Step Function qua VPC Endpoint thì cần setup Security Group của lambda function cho phép outbound traffic đến security group của VPC Endpoint Step Function và ngược lại Security Group của VPC Endpoint cần cho phép inbound traffic từ security group của Lambda
	- Private endpoint: cho phép giao tiếp trực tiếp giữa các resources trong VPC và các dịch vụ AWS mà không cần ra internet. Có 2 loại VPC endpoint:
    	...
    	= 2 cái này khác nhau ở chỗ GWEP sử dụng dns để routing(dig từ prefix ra IP), IFEP sử dụng IP (thông qua ENI) - check lại và viết lại
    
Điểm khác nhau giữa GWEP và IFEP (viết lại)

Interface Endpoint và Gateway Endpoint là hai loại VPC Endpoint của AWS, khác biệt rõ rệt về cách liên kết với route table. Interface Endpoint không yêu cầu thay đổi route table mà dùng ENI và DNS private để định tuyến. Ngược lại, Gateway Endpoint tự động thêm route vào route table được chọn để hướng traffic đến prefix list của dịch vụ.

Interface Endpoint

Khi enable private DNS name khi tạo Interface Endpoint, AWS tự động thêm 1 record trong Route 53 Resolver của VPC, resolve từ service domain (ví dụ: sts.amazonaws.com) thành private IP của ENI thay vì public IP. Application sẽ dùng IP này để connect, traffic chảy nội bộ AWS qua PrivateLink đến service (VD sts) (VPC route table route traffic đến ENI (local subnet, không cần route đặc biệt))

Có thể check bằng cách ssh vào instance và dùng lệnh nslookup sts.us-east-1.amazonaws.com sẽ ra được private IP​

Với IFEP thì không cần thay đổi route table vì traffic đến private IP (RFC 1918) sẽ được route bình thường trong VPC → AWS backbone.​

Lưu ý: khi tạo Interface Endpoint, mỗi subnet bạn chọn sẽ mất 1 private IP từ dải địa chỉ subnet đó để AWS tạo ENI


Gateway Endpoint: Liên kết chặt chẽ với route table khi tạo. AWS tự động thêm route mới vào route table đã chọn: destination là prefix list ID của dịch vụ (như S3/DynamoDB), target là endpoint ID (vpce-xxx). Tất cả instance trong subnet dùng route table đó sẽ tự động dùng endpoint thay vì public endpoint.
...
| - Dùng security group để control traffic                | - Dùng VPC endpoint policy để control traffic               |

#### Lưu ý sau khi tạo VPC Endpoint (dù là Gateway Endpoint hay Interface Endpoint), traffic của đến dịch vụ của AWS sẽ được giữ hoàn toàn trong mạng nội bộ của AWS, không đi ra internet.

Cụ thể:
- Với Interface Endpoint (ví dụ ECR, CloudWatch, Secrets Manager), AWS tạo một ENI trong subnet và traffic đến dịch vụ được gửi trực tiếp qua ENI này trong mạng VPC. DNS tên dịch vụ cũng được ánh xạ tới địa chỉ IP private của ENI, nên lưu lượng không phải đi ra Internet.
- Với Gateway Endpoint (ví dụ S3, DynamoDB), AWS tự động thêm route vào bảng route subnet để mọi traffic đến dịch vụ đó được chuyển hướng qua Endpoint, không qua Internet Gateway hay NAT Gateway.
...
Tạo security group bằng terraform

Security group chặn toàn bộ traffic
```
...
Tạo security group có kèm rule trong lúc tạo
```
resource "aws_security_group" "example" {
  name        = "allow_tls"
  ...
  egress {
    from_port        = 0
    to_port          = 0
    protocol         = "-1" #nghĩa là allow mọi traffic, khi set protocol = -1 thì phải set from_port và to_port = 0
...
Thêm rule vào security group đã tồn tại
```
resource "aws_vpc_security_group_ingress_rule" "allow_tls_ipv4" {
  security_group_id = aws_security_group.allow_tls.id
  cidr_ipv4         = aws_vpc.main.cidr_block
  from_port         = 443
  ip_protocol       = "tcp" #nếu set =-1 thì không set from_port và to_port nữa
  to_port           = 443
}

...
Khi tạo RDS instance bằng resource aws_db_instance trong Terraform, instance này tương đương với một DB instance (hay DB cluster/server) hoàn chỉnh trên AWS RDS. Nó cung cấp endpoint (hostname) mà bạn dùng để kết nối qua lệnh psql.
```
psql \
  --host=<endpoint> \
  --port=5432 \
...
```
​
Instance RDS không phải là một database cụ thể mà là máy chủ PostgreSQL managed, chứa database mặc định postgres và các DB khác bạn tạo sau (qua CREATE DATABASE). RDS aws_db_instance tạo 1 server với DB mặc định postgres. Database khác phải tạo sau bằng CREATE DATABASE myapp. resource aws_db_instance trong Terraform không tự động tạo database khác ngoài postgres

có thể dùng postgresql_database resource từ provider hashicorp/postgresql để tạo database trong RDS PostgreSQL instance đã có.
```
# 1. Cài provider postgresql
terraform {
  required_providers {
    postgresql = {
...
# 3. Tạo database bằng postgresql provider
resource "postgresql_database" "myapp" {
  depends_on          = [aws_db_instance.main]
  name                = "myapp_db"
  owner               = aws_db_instance.main.username
  
  # Kết nối tới RDS endpoint
  provider = postgresql.rds
}

# 4. Config provider connection
provider "postgresql" "rds" {
  host            = aws_db_instance.main.endpoint
  port            = aws_db_instance.main.port
  database        = "postgres"  # Default DB để connect

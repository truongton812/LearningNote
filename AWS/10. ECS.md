...
- Trong đó:
  ...
  – Task definition: là template khai báo các container, cấu hình CPU, memory, network, IAM role, port, volume, biến môi trường. Một task definition có thể chứa một hoặc nhiều container (thường là các microservice hoặc các sidecar container đi kèm).​
...
Khi dùng AWS ECS, các service trong cùng một VPC (và thường là cùng một namespace) có thể giao tiếp với nhau bằng địa chỉ DNS nội bộ, không phải địa chỉ IP cố định của từng container.

Có 2 cách để cho phép giao tiếp giữa các Service
#### 1. Dùng Service Discovery (AWS Cloud Map)

Khi bạn bật Service Discovery cho ECS, mỗi service sẽ có một DNS hostname dạng: `service-name.namespace.svc.cluster.local` và DNS này sẽ được resolver sang IP nội bộ của các task trong service đó.

Bản chất của Service Discovery là đặt tên DNS cho ECS service thông qua AWS Cloud Map. Các client gọi service khác bằng FQDN. Khi sử dụng Service Discovery người dùng cần trả phí DNS query (khoảng 0.40 USD/million DNS query cho tier đầu) và Hosted zone

Cách cấu hình Service Discovery (Cloud Map) trên console
- Bước 1: Tạo namespace trong Cloud Map type: Private DNS namespace (ví dụ tutorial.local) và chọn VPC. Namespace này sẽ tạo domain nội bộ kiểu *.tutorial.local.
​- Bước 2: Tạo ECS service gắn với Service Discovery. Ở phần Service discovery chọn `Enable service discovery integration`, chọn Namespace đã tạo ở bước 1. Set service discovery name. Khi service được tạo, mỗi task sẽ tự đăng ký vào Cloud Map với DNS tương ứng. Các DNS này cũng xuất hiện trong Route53.

#### 2. Dùng ECS Service Connect
Với ECS Service Connect, bạn khai báo tên endpoint cho service (ví dụ: demo), và các service khác trong cùng namespace có thể gọi trực tiếp bằng: `http://demo:8090` (tên ngắn, không cần ghi domain dài). Service Connect sẽ tự dùng Cloud Map + sidecar proxy (Envoy) để định tuyến đến các task đang chạy. 

Khi sử dụng Service Connect người dùng cần trả phí cho Compute cho task (EC2 hoặc Fargate) vì sidecar/agent Service Connect chạy chung trong task.

Cách cấu hình Service Connect trên console
- Bước 1: Trong cấu hình Cluster chỉ định namespace
- Bước 2: Tạo ECS service, trong phần Service connect tích chọn `Enable Service Connect` và chọn namespace
- Bước 3 (optional): Trong bảng Service connect and discovery name configuration (xuất hiện khi chọn Client and server), cấu hình port name và discovery name

Lưu ý không nên dùng cả hai cho cùng một service name vì có thể “đụng” đăng ký tên trong Cloud Map; nhưng có thể dùng song song nếu dùng hai tên khác nhau (một cho DNS-based SD, một cho Service Connect).

---
#### ECS EXEC
ECS Exec là tính năng của AWS ECS cho phép chạy lệnh trực tiếp vào container đang chạy (Fargate hoặc EC2) mà không cần SSH vào host, mở port inbound, hay quản lý SSH keys. ECS Exec tương tự như lệnh docker exec

Cách hoạt động: Dùng AWS Systems Manager (SSM) Session Manager tạo tunnel an toàn từ AWS CLI → container:
- ECS agent bind-mount SSM binaries vào container
- Chạy SSM agent như sidecar process (root user).
- CLI command tạo secure channel qua SSM → exec lệnh.


Để dùng ECS Exec, cần:
- Task dùng network mode awsvpc.
- Agent SSM trong container/task (Agent SSM là thành phần tự động được ECS agent khởi động bên trong container khi bật ECS Exec nên thực chất không phải làm gì)
- AWS CLI v2 và Session Manager plugin trên máy bạn.
- Cấp quyền IAM cần thiết cho task role của ECS task (AmazonSSMManagedInstanceCore các action ssmmessages:*, ssm:UpdateInstanceInformation, logs:CreateLogStream, logs:PutLogEvents nếu dùng CloudWatch log). Task role cần quyền SSM vì SSM agent chạy bên trong container của task và sử dụng credentials tạm thời từ task role để gọi API AWS, đảm bảo bảo mật không hardcode key. SSM agent trong container giúp mở exec session bằng cách gọi các SSM API như ssmmessages:CreateControlChannel, OpenDataChannel... để mở kênh dữ liệu hai chiều từ CLI của bạn đến container.

---
#### Set biến môi trường cho ECS task
Có 2 cách chính để set biến môi trường cho task ECS: cấu hình trong Task Definition hoặc override khi run task/Service.

1. Set env trong Task Definition
- Tạo task definition của bạn, trong phần Container definitions, chọn container cần set env.
- Tìm phần Environment variables (hoặc Environment). Bấm “Add environment variable” rồi nhập cặp key-value
- Nếu có nhiều biến, có thể dùng environmentFiles: Tạo file .env dạng KEY=VALUE rồi upload lên S3. Trong Task Definition → container → Environment → chọn “Add environment file from S3”, chỉ định S3 URL.
​
2. Override khi run task (CLI/API)
Khi gọi aws ecs run-task, có thể truyền overrides để set/override env cho container:
```
aws ecs run-task \
...
Luồng CICD để thay đổi env của task
- Lấy file task-definition.json (hoặc template) trong repo.
- Thay giá trị env từ file config/secret (theo môi trường).
- register-task-definition để tạo revision mới.
- update-service để service dùng revision mới → ECS rolling deploy container mới với env mới.

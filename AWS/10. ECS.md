...
- Hạn chế: không hỗ trợ GPU, chỉ tích hợp với EFS, không hỗ trợ EBS volume, DaemonSet/privileged container, không kiểm soát loại instance vật lý phía dưới. Registry có thể sử dụng là ECR và docker-hub. Cost cao hơn EC2 nếu cluster được sử dụng hiệu quả toàn bộ tài nguyên.
...
-Mỗi task (hoặc mỗi container trong Fargate) được gán một ENI (Elastic Network Interface) và địa chỉ IP riêng trong VPC.
...
host network mode
Container chia sẻ trực tiếp network stack với EC2 instance, không qua NAT hay bridge; port container sẽ là port của host.

Tăng hiệu năng network (giảm độ trễ), nhưng có thể gây xung đột port khi có nhiều container trên một instance.

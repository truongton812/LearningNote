...
## 4. Task execution role và task role
...
Lưu ý: khi tạo ELB cho ECS fargate thì phải tạo target group type là IP (do chỉ có container IP) thì sau này ở Service mới nhận. Tuy nhiên ta không thể biết trước được IP của task để tạo target group cho phù hợp. Vậy nên khi tạo ELB thì tạo 1 target group empty để làm target group mặc định, sau đó khi tạo Service thì tạo target group mới ứng với đường dẫn `/` ⭢ ELB sẽ đẩy traffic đến target group này khi truy cập đến root, còn nếu không match với rule nào thì sẽ đẩy về target group mặc định
...
- Sau đó khi tạo ECS Service ta quy định Service sử dụng target group riêng cho route của mình (ví dụ `/`). Khi đó ECS tự động tạo và quản lý việc đăng ký IP task vào target group đó.  
...
#### 3.1.1. Capacity Provider Strategy

...
- without (label): Gom nhóm và loại bỏ các nhãn chỉ định khỏi kết quả. Nghĩa là khi thực hiện phép tổng hợp (aggregation) như avg, sum,... thì sẽ không phân biệt hoặc giữ lại nhãn (label) được chỉ định
...
Ví dụ: khi giám sát nhiều servers thì metric `node_cpu_seconds_total` có các chuỗi số liệu khác nhau cho từng CPU (cpu0, cpu1, cpu2...) và từng server instances, khi bạn dùng avg without(cpu)(...), Prometheus sẽ gom các instances có cùng nhãn (trừ nhãn cpu) thành 1 nhóm và tính trung bình giá trị `node_cpu_seconds_total` của từng nhóm. Kết quả là ta được trung bình thời gian sử dụng tất cả các core của từng servers

  Tóm lại: Nhãn cpu sẽ bị loại ra khỏi việc phân biệt các chuỗi dữ liệu trong phép aggregator. Các chuỗi chỉ khác nhau ở nhãn cpu sẽ được gộp chung lại với nhau.


###### Ví dụ giải thích cách dùng without(label)
Có 2 query:
- avg(rate(node_cpu_seconds_total{mode="idle"}[5m])) 
- avg without(cpu)(rate(node_cpu_seconds_total{mode="idle"}[5m])) (query `avg by (hostname) (irate(node_cpu_seconds_total{mode="idle"}[5m])*100)` cũng cho ra kết quả tương tự)

Giả sử bạn có nhiều CPU (cpu0, cpu1, cpu2...) trên mỗi máy chủ (instance). Mỗi CPU này sẽ có một dòng số liệu riêng trong metric node_cpu_seconds_total với nhãn cpu.
...
- Sau đó nó tính trung bình, nhưng bỏ qua nhãn cpu. Nghĩa là nó tính trung bình các CPU trong cùng một máy chủ, nhưng khác máy thì giữ riêng.
...
avg(rate(node_cpu_seconds_total{mode="idle"}[5m])) sẽ tính trung bình tổng cộng: (10 + 20 + 30 + 40) / 4 = 25

avg without(cpu)(rate(node_cpu_seconds_total{mode="idle"}[5m])) sẽ tính trung bình nhóm theo máy:
...
- rate(): Tính tốc độ tăng trung bình trên giây của một counter trong 1 khoảng thời gian, chỉ áp dụng với metric kiểu counter. Hàm rate giúp ta nắm được xu hướng của metric đang tăng nhanh hay châm, điều mà bản thân counter metric không thể hiện được. VD khi query metric prometheus_http_requests_total thì sẽ nhận được giá trị số lượng http request gần nhất, giá trị đấy không thể hiện được xu hướng do số lượng http request là 1 số luôn tăng -> rate giúp biết được tốc độ tăng http_request là nhanh hay chậm: `rate(prometheus_http_requests_total[1m])` để tính ra trong 1 giây có bao nhiêu http request, với range là hiện tại đến 1’ trước
  - Cách hàm rate() hoạt động: Prometheus tính ΔV và ΔT là “độ tăng” và “độ dài thời gian”. ΔV bằng sample cuối trừ sample đầu trong khoảng thời gian chỉ định (thực chất Prometheus còn xử lý reset, tuy nhiên phức tạp và có thể bỏ qua). ΔT là độ dài thời gian (tính bằng giây) tương ứng với khoảng mà Prometheus coi là “đang đo”. Rate() trả về giá trị đơn (single value) và bằng ΔV/ΔT
  ...
  - Giải thích thêm về cách tạo graph của hàm rate(): Khi bạn query: rate(http_requests_total[3m]) Prometheus sẽ tính tốc độ trung bình trên mỗi giây của metric http_requests_total trong từng cửa sổ 3 phút kết thúc lần lượt tại mỗi điểm sampling 12:00, 12:01, 12:02, ... Và nếu ta set step trong graph là 1 phút (60s) thì kết quả sẽ là:
...
- irate(): Tốc độ tăng tức thời (tính theo hai điểm dữ liệu gần nhất chứ không phải 2 điểm đầu cuối như rate). Irate dùng để tạo graph của fast moving counter

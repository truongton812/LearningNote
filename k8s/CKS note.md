...
#### ca.crt cũng là cert X.509 bình thường, nhưng có một điểm đặc biệt:

CA tự ký chính mình (self-signed)
```
┌─────────────────────────────────────┐
│         TBSCertificate              │
│  - Issuer:  CN=kubernetes           │  ← chính nó
│  - Subject: CN=kubernetes           │  ← chính nó (Issuer == Subject)
│  - Public Key  ◄────────────────────┼── public key của CA
│  - Extensions:                      │
│      CA: TRUE  ◄────────────────────┼── đánh dấu đây là CA
├─────────────────────────────────────┤
│         Signature                   │  ← ký bằng chính private key của CA
└─────────────────────────────────────┘
```
Issuer == Subject → đây là dấu hiệu self-signed cert.

Không ai verify ca.crt cả, do ca.crt là trust anchor (điểm tin tưởng gốc). Bạn tin nó vì bạn tự tay tạo ra nó 

Verify chính nó: `openssl verify -CAfile /etc/kubernetes/pki/ca.crt /etc/kubernetes/pki/ca.crt # Output: OK`
...
- Khi apply YAML manifest message lỗi sẽ hiện lên, dựa vào đấy để fix. VD:
  - `Error: pods "webapp-5c7f6d5c7f-xyz" is forbidden: violates PodSecurity "restricted:latest": spec.containers[0].securityContext.privileged = true` → Restricted PSS không cho chạy Privileged containers
  - `Error: pods "webapp-5c7f6d5c7f-xyz" is forbidden: violates PodSecurity "restricted:latest": spec.containers[0].securityContext.runAsUser = 0` → Restricted PSS không cho chạy dưới dạng root
  - `Error: pods "webapp-5c7f6d5c7f-xyz" is forbidden: violates PodSecurity "restricted:latest": spec.volumes[0].hostPath = /tmp` → Restricted PSS không cho dùng HostPath volumes
  - `Error: pods "webapp-5c7f6d5c7f-xyz" is forbidden: violates PodSecurity "restricted:latest": spec.containers[0].securityContext.capabilities.add = ["NET_ADMIN"]` → Restricted PSS không cho thêm capabilities
- Cách fix: Sửa securityContext:
  ...
  - readOnlyRootFilesystem: true. Khi set readOnlyRootFilesystem: true, toàn bộ root filesystem (/) của container trở thành read-only. Mọi thao tác ghi vào filesystem đều bị từ chối. Tuy nhiên trong thực tế hầu hết app đều cần ghi vào một vài thư mục (log, cache, temp file...). Giải pháp là mount emptyDir vào đúng những chỗ cần ghi, phần còn lại giữ read-only. Ví dụ một web app cần ghi vào /tmp và /var/cache/app thì bạn cần mount emptyDir đè lên đúng 2 thư mục đấy. (emptyDir là một volume tạm — Kubernetes tạo một thư mục trống trên node, rồi mount đè lên một đường dẫn trong container. Phần được mount đè đó sẽ writable do bản chất mount là đặt một filesystem khác lên trên một đường dẫn, che đi nội dung cũ, không còn liên quan gì đến root filesystem nữa)
  - capabilities: drop: ["ALL"]. Container đang thêm capability NET_ADMIN (quyền thao tác network của node). Restricted chỉ cho phép thêm duy nhất NET_BIND_SERVICE, ngoài ra phải drop ALL.
  - volumes: emptyDir: {}
